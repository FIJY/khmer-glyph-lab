...
function isKhmerIndependentVowel(char) {
  const codePoint = cp(char);
...
function getKhmerConsonantSeries(char) {
  if (!isKhmerConsonantChar(char)) return null;
  return SERIES_O.has(cp(char)) ? 'o_series' : 'a_series';
}

function getKhmerGlyphCategory(char, prevChar = '') {
  const codePoint = cp(char);
  if (isKhmerConsonantChar(char)) {
    return prevChar && cp(prevChar) === KHMER.COENG ? 'subscript_consonant' : 'base_consonant';
  }
  if (isKhmerIndependentVowel(char)) return 'independent_vowel';
  if (isKhmerDependentVowel(char)) return 'dependent_vowel';
  if (codePoint === KHMER.COENG) return 'coeng';
  if (isKhmerDiacriticOrSign(char)) return 'diacritic_sign';
  if (codePoint >= KHMER.NUMERAL_START && codePoint <= KHMER.NUMERAL_END) return 'numeral';
  return 'other';
}
